...
class CustomDPOTrainer(DPOTrainer):
    ...
    def get_batch_loss_metrics(
        ...
        # === Regularization parameters from command line ===
        lambda_reg = self.finetuning_args.lambda_reg
        m = self.finetuning_args.margin_target
        scaling = self.finetuning_args.margin_scaling
        margins = margins / scaling
        m = m / scaling
        sigmoid_reg = lambda_reg * torch.sigmoid(m - margins)
        losses = losses + sigmoid_reg
        
        print(f'margsins.requires_grad: {margins.requires_grad}')
        print(f'margins: {margins.mean().item()}')
        print(f'lambda_reg: {lambda_reg}')
        
        
        print('sigmoid_reg',sigmoid_reg.mean().item())
        print('losses',losses)
        
        print(margins)
        metrics[f"{prefix}rewards/margins"] = margins.tolist()
        print(margins.shape)
        metrics[f"{prefix}rewards/margins_var"] = margins.var(unbiased=False).item()
        ...
        metrics[f"{prefix}reg_mean"] = sigmoid_reg.mean().item()
        metrics[f"{prefix}main_mean"] = (losses.mean() - sigmoid_reg.mean()).item()
        if self.loss_type == "orpo":
